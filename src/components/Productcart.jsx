    ...
    <>
      <div className='bg-gray-900 rounded-2xl shadow-xl overflow-hidden flex flex-col h-full transition duration-500 transform border border-gray-800 group hover:scale-[1.03] hover:shadow-orange-900/40'>
        ...
        <Link to={`/product/${product.id}`} className='relative cursor-pointer overflow-hidden'>
          ...
          <div className='absolute bg-orange-600/95 bottom-0 left-0 text-white px-5 py-1 text-xl font-bold rounded-tr-xl shadow-lg'>
        ...
        <div className='p-5 flex flex-col grow'>
          ...
          <p className='text-gray-400 line-clamp-3 text-sm mb-4 '>{product.description}</p>

          <div className='flex items-center text-xs text-gray-500 mb-4'>
            <span className='px-3 py-1 bg-gray-800 border border-gray-700 rounded-full font-semibold'>{product.category}</span>
          </div>

          <button onClick={() => addtocart(product)} className='mx-auto py-3 w-full bg-orange-600 rounded-full font-bold text-white shadow-lg shadow-orange-800/50 cursor-pointer hover:bg-orange-700 transition duration-200 flex items-center justify-center space-x-2 transform hover:ring-4 hover:ring-green-600/50 uppercase tracking-wider'>
